...
/*
	...
	for (int i=0; i<Number_of_Viruses; ++i)
	{
		...
		Table_of_Viruses_matModel[i] = glm::translate(Table_of_Viruses_matModel[i], glm::vec3(x, y, z) );
	...
	glDrawArraysInstanced( GL_TRIANGLES, 0, OBJ_vertices.size(), Number_of_Viruses);
...
void Initialize()
{
	...
	if (!loadOBJ("virus.obj", OBJ_vertices, OBJ_uvs, OBJ_normals))
	{
		printf("Not loaded!\n");
		exit(1);
	}
	...
	loadBMP_custom("virus.bmp", tex_width, tex_height, &tex_data);
	...
	glBufferData(GL_ARRAY_BUFFER, Number_of_Viruses * sizeof(glm::mat4), &Table_of_Viruses_matModel[0], GL_STATIC_DRAW);
