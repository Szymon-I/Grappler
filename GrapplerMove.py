...
class GrapplerMove:
    ...
    def line_movement(self, line_length=20):
        data = [
            f'0.0/{GrapplerMove.normalizer}/0.0' for _ in range(line_length)]
        data += [
            f'0.0/{-GrapplerMove.normalizer}/0.0' for _ in range(line_length)]
        self.send_data(data)
    ...
    def square_movement(self, line_length=20):
        data = [
            f'0.0/{GrapplerMove.normalizer}/0.0' for _ in range(line_length//2)]
        data += [f'{+GrapplerMove.normalizer}/0.0/0.0' for _ in range(
            line_length//2)]
        data += [
            f'0.0/{-GrapplerMove.normalizer}/0.0' for _ in range(line_length)]
        data += [
            f'{-GrapplerMove.normalizer}/0.0/0.0' for _ in range(line_length)]
        data += [
            f'0.0/{GrapplerMove.normalizer}/0.0' for _ in range(line_length)]
        data += [f'{GrapplerMove.normalizer}/0.0/0.0' for _ in range(
            line_length//2)]
    ...
    def wave_movement(self, line_length=50):
        ...
        data = [
            f'{math.sin(sin_arg[x])*GrapplerMove.normalizer}/{GrapplerMove.normalizer}/0.0' for x in range(line_length)]
        data += [
            f'{-math.sin(sin_arg[x])*GrapplerMove.normalizer}/{-GrapplerMove.normalizer}/0.0' for x in range(line_length)]
        self.send_data(data)
    ...
    def move_once(self, direction):
        data = None
        if direction == 'w':
            data = [f'0.0/{GrapplerMove.normalizer}/0.0']
        elif direction == 's':
            data = [f'0.0/{-GrapplerMove.normalizer}/0.0']
        elif direction == 'a':
            data = [f'{GrapplerMove.normalizer}/0.0/0.0']
        elif direction == 'd':
            data = [f'{-GrapplerMove.normalizer}/0.0/0.0']
        self.send_data(data)
